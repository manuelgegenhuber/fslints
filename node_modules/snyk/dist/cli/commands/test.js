...
var WIZARD_SUPPORTED_PMS = require('../../lib/snyk-test/common').WIZARD_SUPPORTED_PMS;
var SEPARATOR = '\n-------------------------------------------------------\n';
...
function test() {
    ...
    return apiTokenExists('snyk test')
        ...
        var vulnerableResults = results.filter(function (res) {
...
function displayResult(res, options) {
    ...
    if (res.ok && res.vulnerabilities.length === 0) {
        var vulnPathsText = options.showVulnPaths ?
            ...
            ', none were found.';
        var summaryOKText = chalk.green('✓ ' + testedInfoText + vulnPathsText);
        var nextStepsText = '\n\nNext steps:' +
            '\n- Run `snyk monitor` to be notified ' +
            'about new related vulnerabilities.' +
            '\n- Run `snyk test` as part of ' +
            'your CI/test.';
        return (prefix + meta + summaryOKText + (isCI ? '' : dockerAdvice + nextStepsText));
    }
    ...
    var groupedVulnInfoOutput = sortedGroupedVulns.map(function (vuln) {
        ...
        var uniquePackages = _.uniq(vuln.list.map(function (i) {
            ...
            .join(', ');
        var vulnOutput = {
            issueHeading: createSeverityBasedIssueHeading(vuln.metadata.severity, vuln.metadata.type, vuln.metadata.name),
